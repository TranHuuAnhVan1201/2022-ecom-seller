...
export const productApiPr = {
  ...
  getListHighDiscountProduct: (limit) => {
    const url = `/Newee/ProductSeller/GetListHighDiscountProduct?total=${limit}`;
    return axiosClient.get(url);
  },
  getDetail: (id) => {
  ...
  createBillSeller: (data) => {
    const url = `/Newee/Bill/buyer/createBill`;
    return axiosClient.post(url, data);
  },
