...
const FooterHome = lazy(() => import('./app/components/common/footer/FooterHome'))
// seller - admin
...
const Checkout = lazy(() => import('./app/components/_seller/body/checkout/Checkout'))
const Checkout2 = lazy(() => import('./app/components/_seller/body/checkout/Checkout2'))
...
const WhishList = lazy(() => import('./app/components/admin/pages/Users/WhishList'))
const WishList = lazy(() => import('./app/components/_seller/body/wishList'))
const EditBank = lazy(() => import('./app/components/admin/pages/Users/page/editBank/EditBank'))
const ChangePassword = lazy(() => import('./app/components/admin/pages/Users/ChangePassword'))
const Address = lazy(() => import('./app/components/admin/pages/Users/InformationAddress'))
const Sale = lazy(() => import('./app/components/admin/pages/Sale/Sale'))

const Analysis = lazy(() => import('./app/components/admin/pages/analysis/Analysis'))
const AnalysisBank = lazy(() => import('./app/components/admin/pages/analysis/AnalysisBank'))
const Custommer = lazy(() => import('./app/components/admin/pages/customer/Custommer'))

const Banking = lazy(() => import('./app/components/admin/pages/Banking/Banking'))

// custommer

const CustomerHome = lazy(() => import('./app/components/_custommer/HomeCustomer'))
...
// COMMON
const CommonSearch = lazy(() => import('./app/components/common/body/page-search/search/Search'))
const CartEmpty = lazy(() => import('./app/components/_custommer/body/cart/Cart'))
const CartNew = lazy(() => import('./app/components/_seller/body/cart/Cart'))
const CreatePage = lazy(() => import('./app/components/_seller/body/createPage/CreatePage'))
const Checking = lazy(() => import('./app/components/_seller/body/createPage/Checking'))

const FormSaleTogether = lazy(() => import('./app/_pages/supports/saleTogether/FormSaleTogether'))
const SaleTogether = lazy(() => import('./app/_pages/supports/saleTogether/SaleTogether'))
const Policy = lazy(() => import('./app/_pages/supports/policy/Policy'))
const Abouts = lazy(() => import('./app/_pages/supports/abouts/Abouts'))
const Regulations = lazy(() => import('./app/_pages/supports/regulations/Regulations'))
...
const Regulation = lazy(() => import('./app/_pages/supports/_regulations/Index'))
...
function App() {
  ...
  const [token, setToken] = useState(() => {
    let isToken = localStorage.getItem(TOKEN_SELLER) ?? null
    return isToken
  })

  const [idCart, setIdCart] = useState(() => {
    let cart = localStorage.getItem(ID_CARD_SELLER) ?? null
    return cart
  })

  const [isUser, setIsUser] = useState(() => {
    let token = localStorage.getItem(TOKEN_SELLER) ?? null
    let cart = localStorage.getItem(ID_CARD_SELLER) ?? null
    let idUser = localStorage.getItem(ID_USER_SELLER) ?? null
    ...
    if (token && token.length > 0) {
      const FetchPages = async () => {
        try {
          ...
          const cart = await dataLoginApiPr.getDataCart(user?.cart ?? idCart, token)
            ...
            <div className="newee-wrapper mg-85 mg-105">
              <Switch>
                <Route path={'/connect/:token/:cart/:idUser'} exact>
                  <Connect setToken={setToken} setIdCart={setIdCart} setIsUser={setIsUser} />
                </Route>
                ...
                {token && (
                  <Switch>
                    ...
                    <Route path={'/search/:slug.:idCategory'} exact component={CommonSearch} />
                ...
                <Switch>
                  ...
                  <Route path={'/product-detail/:slug.:id'} exact component={DetailCustomer} />
                  <Route path={'/search/:slug.:idCategory'} exact component={CommonSearch} />
                  <Route path={'/search'} exact component={CommonSearch} />
