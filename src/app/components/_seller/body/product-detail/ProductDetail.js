...
import { product_detail, product_detail_variant } from 'v2/data'
...
function ProductDetail(props) {
  ...
  const getProductVariant = async () => {
    try {
      ...
      if (response?.length > 0) {
        const ascending = response.sort((a, b) => Number(a.price) - Number(b.price))
        setVariant(ascending)
      } else {
        setVariant(product_detail_variant)
      }
    } catch (error) {
      setVariant([])
    }
  ...
  const debouncedSave = useRef(debounce((quantity) => saveChangeInput(quantity), 1000)).current
  ...
  const addToCartHandler = async () => {
    ...
    try {
      console.log('stateVariant', stateVariant)
      if (!isLoading) {
        const existItem = cartItems.find(
          (x) => x.idProduct === id && x.idVariant === stateVariant?.currentId,
        )
        const quantity = existItem ? existItem.count * 1 + quantitySaveDb * 1 : quantitySaveDb * 1
        if (existItem) {
          existItem.count = quantity

          await productApiPr.changeQuantityFromCart(existItem.id, quantity)

          const cart = await dataLoginApiPr.getDataCart(user?.cart || dataConnect?.cart)

          dispatch({ type: PRIVATE_CART_LOADING, cart })

          console.log('existItem', existItem)
        } else {
          await productApiPr.addToCart(
          ...
          const cart = await dataLoginApiPr.getDataCart(user?.cart || dataConnect?.cart)
          dispatch({ type: PRIVATE_CART_LOADING, cart })
          console.log('not existItem', existItem)
        }
  ...
  const uiPrivate = (
    <div className="container-detail ">
      <ul className="container-breadcrumb">
        ...
        <li>
          <Link to={`/search/${ToSlug(dataPublic?.brand)}.all`}>{dataPublic?.brand}</Link>
        </li>
      </ul>
            ...
            <div className="content-left">
              <div className="content-left-img">
                    ...
                    variant
                      ...
                      .map((value, key) => (
                        <LazyLoadImage
                          ...
                          alt="Newee"
                          className={
                            key === stateVariant?.current || value.id === stateVariant?.currentId
                ...
                <div className="box-options box-policy mt-2">
                  <li className="py-2 d-flex-center mr-2" onClick={scrollToElement}>
                    <span className="newee-icons pr-1">
                      <HiOutlineShare />
                    </span>
                    Chia sẻ mẫu bán hàng
                  </li>
                  <li className="py-2 d-flex-center mr-2" onClick={scrollToComment}>
                    <span className="newee-icons pr-1">
                      <FaRegCommentDots />
                    </span>
                    Đánh giá
                  ...
                  >
                    <span className="newee-icons red pr-1">
                      {favorites.includes(id) ? <HiHeart /> : <HiOutlineHeart />}
                    </span>
                    Yêu thích
                  </li>
                </div>
            ...
            <div className="content-right">
                    ...
                    <strong className="box-money-received box-price-current">
                      {stateVariant.selected >= 0
                        ? variant[stateVariant.selected].percent +
                          '%' +
                          ' (' +
                          FormatVND(variant[stateVariant.selected].moneyReceived) +
                          '₫' +
                          ')'
                        : stateVariant.current >= 0
                          ? variant[stateVariant?.current].percent +
                            '%' +
                            ' (' +
                            FormatVND(variant[stateVariant?.current].moneyReceived) +
                            '₫' +
                            ')'
                          : variant[0].percent +
                            '%' +
                ...
                <div className="box-options-flex">
                  {variant?.map((value, key) => (
                    <li
                      ...
                      className={
                        key === stateVariant?.selected || value.id === stateVariant?.currentId
              ...
              <div className="box-options mt-3">
                ...
                <div className="box-options-flex-0 box-quantity bg-white ">
                  <button className="btn-quantity" onClick={handleQuantityReduced}>
                    -
                  </button>
                  <input
                  ...
                  ></input>
                  <button className="btn-quantity" onClick={handleQuantityIncrease}>
                    +
                  </button>
                </div>

                <label style={{ flex: 1 }}>
                  {variant[stateVariant.selected]?.count || 0} sản phẩm có sẵn
                </label>
              </div>
      ...
      <div className="container-des">
        <div className="container-left av">
          <div className="container-left-content">
            ...
            <pre className="container-des-pre p15-20">{dataPublic?.description}</pre>
          </div>
                      ...
                      variant
                        ...
                        .map((value, key) => (
                          <LazyLoadImage
                            ...
                            alt="Newee"
                            className={
                              key === selectedContent?.current || key === selectedContent?.selected
                                ? 'slide-bottom-image image-selected'
                                : 'slide-bottom-image'
                            }
                            onMouseEnter={() => handleSelectedImages(key, value.imageLink)}
                            onMouseLeave={() => handleUnSelectedImages(key, value.imageLink)}
                            onClick={() => handleClickImages(key, value.imageLink)}
                          />
                        ))}
                  </div>
                </div>
  ...
  const content = (
    <section className="">
      <MetaDecorator
        description={
          (dataPublic && dataPublic?.description?.substr(0, 150)) || 'Newee asia - Happy Seller'
        }
        title={(dataPublic && dataPublic?.name) || 'Newee asia - Happy Seller - title'}
        imageUrl={(dataPublic && dataPublic?.link) || 'https://seller.newee.asia'}
