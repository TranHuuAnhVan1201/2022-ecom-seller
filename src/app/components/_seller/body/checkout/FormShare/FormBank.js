...
import { checkoutApi } from "../../../../../../api/private";
...
function FormBank(props) {
  ...
  const createTypeBank = async () => {
    try {
      const response = await checkoutApi.createBill(data);
      ...
      setLoading(false);
      setIdBill(error.code);
      setBill(error.totalPrice + error.priceShip);
      console.log("đã lỗi đây này", error);
    }
  };
