...
function Search(props) {
  ...
  const handleTab = (tab) => {
    ...
    if (tab === 0) {
      ...
      console.log(list)
      let ascending = list.sort((a, b) => Number(a.percent) - Number(b.percent))
      setList(ascending)
    } else if (tab === 5) {
      let ascending = list.sort((a, b) => Number(b.percent) - Number(a.percent))
  ...
  const handleSort = (key, price1, price2) => {
    setActivePrice(key)
    if (FetchProduct) {
      var d = FetchProduct.filter((e) => price1 <= e.price1 && e.price1 < price2)
  ...
  useEffect(() => {
    ...
    setLoading(true)
    if (Shop.results === null || Shop.results === [] || Shop.results === undefined) {
      setList(null)
  ...
  const getProductPublicWidthCategory = (idCategoryPublic, total, indexPage) => {
